...
/* cau 19: SU DUNG VIEW */
IF EXISTS(SELECT * FROM [SUM_CTHD]) DROP VIEW [SUM_CTHD]
GO
CREATE VIEW [SUM_CTHD]
AS
SELECT SOHD, SUM(SL) AS TOTAL FROM (SELECT DISTINCT(SOHD), SL FROM CTHD AS CT1 WHERE NOT EXISTS (
	SELECT MASP FROM SANPHAM WHERE NUOCSX = 'Singapore' AND NOT EXISTS (SELECT SOHD FROM CTHD AS CT2 WHERE CT1.SOHD = CT2.SOHD AND SANPHAM.MASP = CT2.MASP))
) AS T
GROUP BY SOHD
GO
SELECT SOHD FROM [SUM_CTHD] WHERE TOTAL <= (SELECT MIN(TOTAL) FROM [SUM_CTHD])
