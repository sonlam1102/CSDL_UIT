...
/* cau 19 */
SELECT DISTINCT SOHD, MIN(SL) AS SL FROM CTHD AS HD WHERE SOHD NOT IN (
SELECT SOHD FROM (SELECT MASP FROM SANPHAM AS SP WHERE NUOCSX = 'Singapore' EXCEPT SELECT MASP FROM CTHD AS HD2 INNER JOIN HOADON ON HD2.SOHD = HOADON.SOHD WHERE HD.SOHD = HD2.SOHD) AS R
)
GROUP BY SOHD
